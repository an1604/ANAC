...
class DetectionRegion:
    def __init__(self, n_clusters, deadline_time, initial_value, time_low_bound,
        ...
        self.Kmeans = KMeans(n_clusters=n_clusters, init='k-means++', random_state=RANDOM_STATE)
    ...
    def initialize_cells_division(self, first_init, X=None, X_labels=None):
        ...
        for idx, cell_center in enumerate(self.Kmeans.cluster_centers_):
            ...
            time_high = cell_center[0] + (self._DetReg['time_high'] - cell_center[0]) / 2
            value_low = cell_center[1] - (cell_center[1] - self._DetReg['reserved_value']) / 2
            value_high = cell_center[1] + (self._DetReg['initial_value'] - cell_center[1]) / 2
            ...
            if first_init:
                ...
                posterior_probability = np.random.uniform(low=self._eps, high=2 * self._eps)

            else:  # Means that self_cells already initialized
                contains, cell_associated = self.get_right_or_closest_cell(cell_center, cell_range)
                if contains:
                    ...
                    prior_probability = cell_associated.prob.prior_probability - self._eps
                    likelihood = cell_associated.prob.likelihood_probability - self._eps
                    posterior_probability = cell_associated.prob.posterior_probability - self._eps
            ...
            cells.append(
                Cell(
                    ...
                    offers_associated=np.array([X[i] for i in range(len(X)) if X_labels[i] == idx])
                ))
    ...
    def generate_X_offers(self, n_points=100000) -> np.array:
        ...
        for cluster_index in cluster_indices:
            ...
            for _ in range(points_per_cluster):
                cluster_mean = (cluster_ranges[cluster_index] + cluster_ranges[cluster_index + 1]) / 2
                cluster_stdev = (cluster_ranges[cluster_index + 1] - cluster_ranges[cluster_index]) / 6
    ...
    def update(self):
        ...
        if len(cells_in_boundaries) < self.n_cells:
            ...
            self.initialize_cells_division(first_init=False, X=X,
                                           X_labels=X_labels)  # Calling the cells initialization function
    ...
    def get_the_most_recent_cell_index(self, new_center_time):
        ...
        valid_centers = [center for label, center in zip(labels, cluster_centers)
        ...
        if len(valid_centers) == 0:
            ...
            min_cluster = min([center for center in cluster_centers], key=lambda x: x[0])
    ...
    def get_estimated_rv(self) -> np.ndarray:
        ...
        return np.array([(t_low + t_high) / 2., (p_low + p_high) / 2.])

    def get_DetReg_bounds_in_ndarray(self) -> np.array:
        return np.array([self._DetReg['time_low'], self._DetReg['time_high'], self._DetReg['initial_value'],
    ...
    def generate_random_reservation_points(self) -> list[tuple[int, np.ndarray]]:
        ...
        if rand_choice in [0, 1]:
            return [tuple([cell.cell_index, cell.cluster_center] for cell in self._cells)]
        else:
            return [
                tuple([cell.cell_index, get_random_rv(time_lower_bound=cell.cell_range[0],
    ...
    def get_right_or_closest_cell(self, cell_center, cell_range=None) -> tuple[bool, Cell]:
        """
        Checks if some cell center is in self._cell and returns it,
        and if not, returns the closest cell that kmeans predicts.
        """
        for cell in self._cells:
            if (cell_center[0] == cell.cluster_center[0] and
                    cell_center[1] == cell.cluster_center[1]):
        ...
        offer_in_cell = get_random_rv(time_lower_bound=cell_range[0], time_upper_bound=cell_range[1],
                                      price_lower_bound=cell_center[2], price_upper_bound=cell_center[3])
        cell_pred = self.Kmeans.predict([offer_in_cell])[0]  # The nearest cell predicted.
        for cell in self._cells:
        ...
        for cell_index, correlation_coefficient_value in non_linear_correlation_coefficient.items():
            ...
            if cell:
                cell.prob.update_likelihood_probability(
                    correlation_coefficient_value)  # Update the likelihood according to the correlation coefficient
